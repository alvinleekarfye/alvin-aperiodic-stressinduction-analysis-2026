...
anova(gam_model_beta)
gam.check(gam_model_beta)

print(gam_model_beta$df.residual)
skewness(resid(gam_model_beta))
kurtosis(resid(gam_model_beta))

sim <- simulateResiduals(fittedModel = gam_model_beta, re.form = NULL, n = 10000)
plot(sim)  

gam_model_beta_2 <- gam(
  Residual_Beta_bc ~ 
    ROI + Time + Age + Gender + Cortisol_Baseline +
    s(Subject, bs = "re"),  
  data = df_combined,
...
summary(gam_model_beta_2)
anova(gam_model_beta_2)
gam.check(gam_model_beta_2)

print(gam_model_beta_2$df.residual)
skewness(resid(gam_model_beta_2))
kurtosis(resid(gam_model_beta_2))

sim <- simulateResiduals(fittedModel = gam_model_beta_2, re.form = NULL, n = 10000)
plot(sim)  

emm_roi <- emmeans(gam_model_beta_2, ~ ROI)
...
posthoc <- emmeans(gam_model_beta, revpairwise ~ Time | ROI, adjust = "none") 
...
p16 <- ggplot(posthoc_df, aes(x = estimate, y = ROI)) +
...
p16 <- p16 + coord_cartesian(xlim = c(-0.2, 0.2))

plot_emm <- emmeans(gam_model_beta, ~ Time | ROI)
...
p17 <- topoplot(df_training, 
                quantity = "quantity", 
                chan_marker  = "name",
                method = "Biharmonic",
                head = TRUE,
                palette = "YlOrRd",
                fill_title="Residual Beta\nBaseline-corrected")
p17 <- p17 +
...
p17 <- p17 + labs(title = "Training") +
  theme(plot.title = element_text(hjust = 0.5, size = 20))
p17

df_stress <- subset(df_topo, segment == "stress")
p18 <- topoplot(df_stress, 
                quantity = "quantity", 
                chan_marker  = "name",
                method = "Biharmonic",
                head = TRUE,
...
p18 <- p18 + labs(title = "Stress") +
  theme(plot.title = element_text(hjust = 0.5, size = 20))
p18

combined <- p16 + p17 + p18 +
