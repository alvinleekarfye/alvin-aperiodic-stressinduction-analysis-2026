...
summary(gam_model_theta)
anova(gam_model_theta)
gam.check(gam_model_theta)

print(gam_model_theta$df.residual)
skewness(resid(gam_model_theta))
kurtosis(resid(gam_model_theta))

sim <- simulateResiduals(fittedModel = gam_model_theta, re.form = NULL, n = 10000)
plot(sim)   

posthoc <- emmeans(gam_model_theta, revpairwise ~ Time | ROI, adjust = "none") 
...
p10 <- ggplot(posthoc_df, aes(x = estimate, y = ROI)) +
...
p10 <- p10 + coord_cartesian(xlim = c(-0.2, 0.2))

plot_emm <- emmeans(gam_model_theta, ~ Time | ROI)
...
p11 <- topoplot(df_training, 
                quantity = "quantity", 
                chan_marker  = "name",
                method = "Biharmonic",
                head = TRUE,
                palette = "YlOrRd",
                fill_title="Residual Theta\nBaseline-corrected")
p11 <- p11 +
...
p11 <- p11 + labs(title = "Training") +
  theme(plot.title = element_text(hjust = 0.5, size = 20))
p11

df_stress <- subset(df_topo, segment == "stress")
p12 <- topoplot(df_stress, 
                quantity = "quantity", 
                chan_marker  = "name",
                method = "Biharmonic",
                head = TRUE,
...
p12 <- p12 + labs(title = "Stress") +
  theme(plot.title = element_text(hjust = 0.5, size = 20))
p12

combined <- p10 + p11 + p12+
