...
summary(gam_model_expo)
anova(gam_model_expo)
gam.check(gam_model_expo)

print(gam_model_expo$df.residual)
skewness(resid(gam_model_expo))
kurtosis(resid(gam_model_expo))

sim <- simulateResiduals(fittedModel = gam_model_expo, re.form = NULL, n = 10000)
plot(sim)

posthoc <- emmeans(gam_model_expo, revpairwise ~ Time | ROI, adjust = "none") 
...
p1 <- ggplot(posthoc_df, aes(x = estimate, y = ROI)) +
...
p1 <- p1 + coord_cartesian(xlim = c(-0.2, 0.2))

plot_emm <- emmeans(gam_model_expo, ~ Time | ROI)
...
p2 <- topoplot(df_training, 
               quantity = "quantity", 
               chan_marker  = "name",
               method = "Biharmonic",
               head = TRUE,
               palette = "YlOrRd",
               fill_title="Exponent (1/f slope)\nBaseline-corrected")
p2 <- p2 +
